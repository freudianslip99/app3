...
<v-form v-model="valid">
  ...
  <v-container grid-list-md>
    <v-layout row wrap align-center>
      <v-flex xs2>
        <v-text-field id="from" name="from" label="From" :rules="[rules.required]" clearable v-model.number="from" type="number">
        </v-text-field>
      </v-flex>
      <v-flex xs2>
        <v-text-field id="to" name="to" label="To" :rules="[rules.required]" clearable v-model.number="to"></v-text-field>
      </v-flex>
      <v-flex xs2>
        <v-text-field id="steps" name="steps" label="Steps" :rules="[rules.required]" clearable v-model.number="steps"></v-text-field>
      </v-flex>
    </v-layout>
    <v-layout row>
      <v-flex xs8>
